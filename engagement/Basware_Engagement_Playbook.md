...
**How to use the companion technical reference:** this playbook tells you what to do and deliver; the [Databricks Lakehouse Data Modeling — Playbook](../databricks/Databricks_Data_Modeling_Playbook.md) provides the detailed options when the walkthrough raises a modeling, reconciliation, Lakeflow, or future-Discovery question; the [Metric Workshop](./Metric_Workshop.md) is the case-clinic visual sequence and templates. Do not duplicate its technical research here. Use [How to use this reference](../databricks/Databricks_Data_Modeling_Playbook.md#how-to-use-this-reference) to get to the right modeling section, then record the chosen pattern and evidence in [Appendix A — KPI Definition Contract](#appendix-a--kpi-definition-contract) and [Appendix B — RAID and Decision Log](#appendix-b--raid-and-decision-log).
...
- Use the [Snowflake ↔ Databricks terminology cheat sheet](../databricks/Snowflake_Databricks_CheatSheet.md) as a live reference during meetings — don't hide it, a data architect cross-walking terminology in real time reads as rigor, not weakness.
- Internal flag (never say this to Basware): the presale risk notes already question whether Metric Views/Genie are mature enough for Basware's actual business-logic complexity. If that risk surfaces during your walkthrough, note it in your internal findings — don't assert it to the client as fact; validate it against what you actually see, using the [Unity Catalog Metric Views architecture brief](../databricks/Metric_Views_Brief.md) as the evidence pack.
...
- [ ] Deliver the KPI walkthrough summary: business definition, lineage, current implementation, evidence pack, and named decisions — in enough detail that it stands alone (this is literally the SOW's acceptance bar: "enough detail for the Cresco team to act without further clarification"). Use [Appendix D — KPI Evidence Pack](#appendix-d--kpi-evidence-pack) and mark every conclusion as **confirmed**, **hypothesis to validate**, **illustrative pattern**, or **decision pending**; the companion [Evidence and claim convention](../databricks/Databricks_Data_Modeling_Playbook.md#evidence-and-claim-convention) defines these labels.
...
- [ ] **Use the least-complex valid path for the SAP partner issue.** First decide whether it is a deterministic hierarchy/role mapping, a known-entity source reconciliation problem, or a genuine entity-matching problem. Use the detailed decision pattern in [Classify the problem first](../databricks/Databricks_Data_Modeling_Playbook.md#classify-the-problem-first); do not recommend probabilistic matching merely because several systems are involved.
- [ ] **Add a safety and acceptance plan for any proposed change** using [Appendix F — Proposed Change Safety and Acceptance Plan](#appendix-f--proposed-change-safety-and-acceptance-plan). Reference [Lakeflow pipeline design](../databricks/Databricks_Data_Modeling_Playbook.md#3-lakeflow-pipeline-design) and [From source disagreement to Gold definition](../databricks/Databricks_Data_Modeling_Playbook.md#4-from-source-disagreement-to-gold-definition) for technical control patterns.
...
- Sample reconciliation results: population size, disagreement categories/counts, and representative records or safely redacted examples — preferably as [Metric Workshop](./Metric_Workshop.md) calculation cards and a source strip.
...
- Do not recommend probabilistic entity matching before [Appendix C — KPI Decision Rights](#appendix-c--kpi-decision-rights) owners and the [Classify the problem first](../databricks/Databricks_Data_Modeling_Playbook.md#classify-the-problem-first) path establish that deterministic options are inadequate.
...
For each scenario, prepare a one-page case card from the [Metric Workshop](./Metric_Workshop.md#3-one-contract-calculation-card): business event, safely redacted record journey, current KPI result, expected result, relevant source events, and open question. Work through the case with the right people:
